...
the server-generated `promptId`; it is never trusted as the server id,
...
is idempotent and returns `not_found`. The public method accepts only the
canonical UUID server ids generated by these queue owners; a malformed or
oversized id gets `INVALID_PARAMS` before lookup. Removal leaves a hidden
...
private inspection races the existing mid-turn-drain timeout and the Session
close signal, so it cannot hold the prompt response indefinitely.
...
record after an awaited private inspection.
...
- The private `craft/cancelPendingPrompt` path used by the daemon child keeps
  running-only semantics.
...
added/started/completed events and snapshots. The bridge continues to generate
and trust only its `promptId` and bridge-issued `clientId`. It revalidates
`clientPromptId`. Each `SessionEntry` gains `admittedPromptIds` and
...
reaper fence. Caller settlement clears the entry's deadline timer. After
